...
import { graphState, RUNTIME_GRAPH_SYNC_COALESCE_MS } from './sync-runtime-graph/graph-state'
import {
...
export function hasRegisteredRuntimeTerminalTab(tabId: string): boolean {
  return graphState.registeredTabs.has(tabId)
}

export function registerRuntimeTerminalTab(tab: RegisteredTerminalTab): () => void {
  graphState.registeredTabs.set(tab.tabId, tab)
  graphState.tabRegisteredAt.set(tab.tabId, Date.now())
  scheduleRuntimeGraphSync()
  return () => {
    // React can mount a replacement before the old effect cleans up.
    if (graphState.registeredTabs.get(tab.tabId) !== tab) {
      return
    }
    graphState.registeredTabs.delete(tab.tabId)
...
export function focusRuntimeTerminalSurface(tabId: string, leafId?: string | null): boolean {
  const registered = graphState.registeredTabs.get(tabId)
